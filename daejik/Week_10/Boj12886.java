...
public class Boj12886 {
	static Queue<Stones> queue = new LinkedList<Stones>();
	...
	public static void main(String[] args) throws IOException {
		...
		while(!queue.isEmpty()) {
			...
			if(now.aGroup != now.bGroup) {
				...
				if(isNotNegative(next) == true && isVisited(next) == false) {
					queue.add(next);
				}
			}
	...
	public static boolean isNotNegative(Stones stones) {
		if(stones.aGroup >= 0 && stones.bGroup >= 0 && stones.cGroup >= 0) return true;
		return false;
	}
	public static boolean sameNumAll(Stones stones) {
		if(stones.aGroup == stones.bGroup && stones.aGroup == stones.cGroup && stones.bGroup == stones.cGroup) return true;
		return false;
	}
	public static boolean isVisited(Stones stones) {
		if(visited[0][stones.aGroup] == true && visited[1][stones.bGroup] == true && visited[2][stones.cGroup] == true) return true;
